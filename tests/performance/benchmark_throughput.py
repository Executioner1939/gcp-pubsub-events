...
class PerformanceMetrics:
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            'avg_processing_time_ms': statistics.mean(self.processing_times) * 1000 if self.processing_times else 0,
            'median_processing_time_ms': statistics.median(self.processing_times) * 1000 if self.processing_times else 0,
            'p95_processing_time_ms': statistics.quantiles(self.processing_times, n=20)[18] * 1000 if len(self.processing_times) >= 20 else 0,
            'p99_processing_time_ms': statistics.quantiles(self.processing_times, n=100)[98] * 1000 if len(self.processing_times) >= 100 else 0,
        }


def run_throughput_benchmark(message_count: int = 1000, payload_size: int = 1024):
    """Run throughput benchmark."""
    print(f"🚀 Running throughput benchmark: {message_count} messages, {payload_size} bytes payload")
    ...
    print(f"📤 Publishing completed in {publish_time:.2f}s ({message_count/publish_time:.1f} msg/s)")
