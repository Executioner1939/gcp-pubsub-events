    ...
    def test_fastapi_with_real_pubsub(self, pubsub_publisher, test_topic, test_subscription):
        ...
        def get_events():
            return {
                ...
                "events": [e.model_dump() for e in listener.events],
            }
        ...
        def get_status():
            return {
                "pubsub_running": (
                    app.state.pubsub.is_running if hasattr(app.state, "pubsub") else False
                )
            }
        ...
        with TestClient(app) as client:
            ...
            test_event = UserEvent(
                user_id="test-user", action="test-action", metadata={"test": True}
            )
