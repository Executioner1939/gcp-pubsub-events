...
class TestCompleteWorkflow:
    ...
    def test_user_order_payment_workflow(self, pubsub_client, pubsub_publisher, 
                                       project_id, run_client, wait_for):
        ...
        class WorkflowEventHandler:
            ...
            async def handle_user_registration(self, event: UserRegisteredEvent, ack: Acknowledgement):
            ...
            async def handle_payment_processing(self, event: PaymentProcessedEvent, ack: Acknowledgement):
        ...
        def workflow_complete():
            return (
                user_id in workflow_state.users and
                len(workflow_state.orders) > 0 and
                len(workflow_state.payments) > 0 and
                workflow_state.workflow_completed.get(user_id, False)
            )
    ...
    def test_error_recovery_workflow(self, pubsub_client, pubsub_publisher,
                                   project_id, run_client, wait_for):
