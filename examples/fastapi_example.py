...
async def lifespan(app: FastAPI):
    ...
    async with async_pubsub_manager(
        ...
        logger.info(
            f"Listening on subscriptions: {USER_EVENTS_SUBSCRIPTION}, {ORDER_EVENTS_SUBSCRIPTION}"
        )
...
def read_root():
    ...
    return {
        ...
        "status": "running",
        "docs": "/docs",
        "endpoints": {"stats": "/stats", "events": "/events", "health": "/health"},
    }
...
def health_check():
    ...
    pubsub_running = hasattr(app.state, "pubsub_manager") and app.state.pubsub_manager.is_running
...
def get_event_stats():
    ...
    if user_service.processed_events or order_service.processed_events:
        user_last = (
            user_service.processed_events[-1].timestamp
            if user_service.processed_events
            else datetime.min
        )
        order_last = (
            order_service.processed_events[-1].timestamp
            if order_service.processed_events
            else datetime.min
        )
...
async def simulate_user_event(event: UserEvent):
    ...
    try:
        ...
        return {"status": "processed", "event_id": event.user_id, "action": event.action}
    except Exception as e:
...
async def simulate_order_event(event: OrderEvent):
    ...
    try:
        ...
        return {"status": "processed", "order_id": event.order_id, "status_event": event.status}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
