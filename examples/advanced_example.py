...
class UserEventService:
    ...
    async def on_user_registered(
        self, event: UserRegisteredEvent, acknowledgement: Acknowledgement
    ):
...
class PaymentEventService:
    ...
    async def on_payment_completed(
        self, event: PaymentCompletedEvent, acknowledgement: Acknowledgement
    ):
        """Handle payment completion events."""
        try:
            self.logger.info(
                f"Processing payment completion: {event.payment_id} - ${event.amount} {event.currency}"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        valid_event = UserRegisteredEvent(
            event_id="evt_123", user_id="user_456", email="user@example.com", username="valid_user"
        )
